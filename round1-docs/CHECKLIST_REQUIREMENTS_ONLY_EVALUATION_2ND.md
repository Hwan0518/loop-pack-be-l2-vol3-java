...
| S2 | 1 | `loginId`, `password`, `name`, `birthDate`, `email` 필수 검증 존재 |
| S3 | 1 | 성공 시 `201 Created` 반환 |
| S4 | 1 | 성공 응답에 `id`, `loginId`, `name`, `birthDate`, `email` 반환 |
...
| S20 | 1 | `password` 생년월일 포함 시 `PASSWORD_CONTAINS_BIRTH_DATE(400)` |
...
| S25 | 1 | `birthDate` 형식 `yyyy-MM-dd`(ISO-8601) 경로 충족 |
| S26 | 1 | `birthDate`를 `1900-01-01` 이후, 오늘 이전으로 검증 |
| S27 | 1 | `birthDate` 위반 시 `INVALID_BIRTH_DATE(400)` |
...
| M3 | 1 | 응답 필드 `loginId`, `name`, `birthDate`, `email` 반환 |
...
| E7 | 1 | `PASSWORD_CONTAINS_BIRTH_DATE = 400 / PASSWORD_CONTAINS_BIRTH_DATE / 비밀번호에 생년월일을 포함할 수 없습니다.` 일치 |
...
| E11 | 1 | `INVALID_BIRTH_DATE = 400 / INVALID_BIRTH_DATE / 생년월일은 1900-01-01 이후, 오늘 이전이어야 합니다.` 일치 |
