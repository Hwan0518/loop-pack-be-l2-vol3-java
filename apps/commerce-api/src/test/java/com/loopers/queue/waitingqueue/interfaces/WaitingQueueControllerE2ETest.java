...
class WaitingQueueControllerE2ETest {
	...
	// unique topic: default topic residue 방지
	private static final String CONTROLLER_E2E_TOPIC =
		"waiting-queue-topic-controller-" + UUID.randomUUID().toString().replace("-", "");

	@DynamicPropertySource
	static void overrideTopic(DynamicPropertyRegistry registry) {
		registry.add("queue.waiting.topic", () -> CONTROLLER_E2E_TOPIC);
	}
	...
	class PositionTest {
		...
		void getPosition_inQueue_returnsPosition() throws Exception {
			...
			Long userId = queueAuthPort.resolveAndRefresh(queueToken).userId();
