...
class UserQueryControllerE2ETest {
	...
	class GetMeTest {
		...
		@Test
		@DisplayName("[GET /api/v1/users/me] 존재하지 않는 loginId -> 404 Not Found")
		void getMeFailWhenUserNotFound() throws Exception {
			...
			mockMvc.perform(get("/api/v1/users/me")
				...
				.andExpect(jsonPath("$.code").value(ErrorType.USER_NOT_FOUND.getCode()));
		}
		...
		@Test
		@DisplayName("[GET /api/v1/users/me] loginId 앞뒤 공백 포함 -> 404 Not Found (원문 loginId 인증)")
		void getMeFailWithWhitespaceLoginId() throws Exception {
			// Arrange
			signUpUser("testuser01", "Test1234!", "홍길동",
				LocalDate.of(1990, 1, 15), "test@example.com");
	...
	class ChangePasswordTest {
		...
		@Test
		@DisplayName("[PATCH /api/v1/users/me/password] X-Loopers-LoginId 헤더 누락 -> 400 Bad Request")
		...
		@Test
		@DisplayName("[PATCH /api/v1/users/me/password] X-Loopers-LoginPw 헤더 누락 -> 400 Bad Request")
		...
		@Test
		@DisplayName("[PATCH /api/v1/users/me/password] newPassword == currentPassword(header password) -> 400 Bad Request. "
		...
		@Test
		@DisplayName("[PATCH /api/v1/users/me/password] 존재하지 않는 loginId -> 404 Not Found")
