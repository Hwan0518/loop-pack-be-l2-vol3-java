 ...
 * - deletedAt: 삭제 일시
 */
public record AdminProductResponse(Long id, Long brandId, String brandName, String name,
	BigDecimal price, Long stock, Long likeCount, ZonedDateTime deletedAt) {

	// 1. AdminProductOutDto를 컨트롤러 응답 객체로 변환
	public static AdminProductResponse from(AdminProductOutDto outDto) {
		return new AdminProductResponse(
			outDto.id(), outDto.brandId(), outDto.brandName(), outDto.name(),
			outDto.price(), outDto.stock(), outDto.likeCount(), outDto.deletedAt()
		);
	}
