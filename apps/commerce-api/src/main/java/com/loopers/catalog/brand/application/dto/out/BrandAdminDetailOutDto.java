 ...
 * - visibleStatus: 노출 상태
 */
public record BrandAdminDetailOutDto(Long id, String name, String description, VisibleStatus visibleStatus) {

	// 1. Brand 도메인 객체를 관리자 상세 조회 결과 DTO로 변환
	public static BrandAdminDetailOutDto from(Brand brand) {
		return new BrandAdminDetailOutDto(
			brand.getId(),
			brand.getName().value(),
