 ...
 * - visibleStatus: 노출 상태
 */
public record AdminBrandOutDto(Long id, String name, VisibleStatus visibleStatus) {

	// 1. Brand 도메인 객체를 관리자 목록 조회 결과 DTO로 변환
	public static AdminBrandOutDto from(Brand brand) {
		return new AdminBrandOutDto(brand.getId(), brand.getName().value(), brand.getVisibleStatus());
	}
