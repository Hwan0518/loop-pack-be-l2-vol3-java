...
- 주의 — Lua script의 `return` 값은 `results`(userId만)이어야 한다. `ZPOPMIN` 결과(`users`)를 그대로 반환하면 `[userId, score, userId, score, ...]` 쌍 배열이 되어 Java 측에서 score(Double)를 String으로 캐스팅할 때 런타임 예외 발생.
...
## 18. enterReceipt — Kafka Consumer 처리 지연 구간 처리

**결론**: `POST /queue/enter` 성공 시 HMAC 서명 기반 진입 증명(`enterReceipt`)을 발급하고, `GET /queue/position`에서 이를 수신해 Consumer 처리 지연 구간을 `PROCESSING` 상태로 응답한다.

**배경 — 문제 구간:**

```
t=0ms   POST /queue/enter → Kafka produce 성공 → "ACCEPTED"
t=1ms   GET /queue/position → Consumer 아직 Redis ZADD 미처리
           ZRANK = null, entry-token = null
           → "QUEUE_NOT_ENTERED 에러"??  ← UX 모순
```

Kafka-first 아키텍처에서 Consumer의 Redis ZADD 처리까지는 짧은 지연이 존재한다.
이 구간에서 에러를 반환하면 유저는 "ACCEPTED" → 즉시 "미진입" 에러라는 모순된 경험을 받는다.
클라이언트가 이를 진짜 에러로 처리하면 재진입 → 뒤로 밀림(NX 미사용 정책 훼손).

**설계:**

```
enterReceipt 형식: "enter:{userId}:{enterTimestamp}:{hmacHex}" (4파트)
- HMAC-SHA256 서명 → 위조 불가
- TTL: tokenTtlSeconds (300초) — 진입 증명이 만료되면 QUEUE_NOT_ENTERED로 낙하

GET /queue/position 상태 판단:
  1. ZRANK 있음            → WAITING  (position, ETA, retryAfterMs)
  2. ZRANK null + 토큰 있음 → TOKEN_ISSUED  (entryToken)
  3. ZRANK null + cap 초과  → QUEUE_CAPACITY_EXCEEDED 에러
  4. ZRANK null + 유효한 enterReceipt → PROCESSING  (2초 후 재시도)
  5. ZRANK null + 모두 없음 → QUEUE_NOT_ENTERED 에러
```

- `enterReceipt`는 stateless — Redis 장애 시에도 동작.
- `PROCESSING` 상태는 Consumer 처리 지연(정상 흐름)과 미진입(비정상)을 구분하는 유일한 수단.
- TTL 300초 만료 후 → 상태 5로 낙하 → 유저에게 재진입 안내 (정상적인 Kafka 장애 처리).

---

## 19. order-lock — 입장 토큰의 TOCTOU 동시성 처리

**결론**: 토큰 검증 직후 Redis SET NX EX로 주문 처리 락을 획득하여, 동일 입장 토큰으로 동시에 복수 주문이 생성되는 것을 방지한다.

**배경 — 문서 초안의 TOCTOU 취약점:**

```
Thread A: GET entry-token → "entry-abc" 확인됨 ✓
Thread B: GET entry-token → "entry-abc" 확인됨 ✓  (A가 아직 DEL 안 함)
Thread A: 주문 처리 완료 → DEL entry-token
Thread B: 주문 처리 완료 → (다른 requestId면) 두 번째 주문 생성
```

유저가 버튼을 두 번 누르거나 네트워크 재시도가 발생할 때, 하나의 입장권으로 두 개의 주문이 생성될 수 있다. `userId + requestId` 멱등성만으로는 동일 requestId race만 커버하고, 다른 requestId 동시 주문은 막지 못한다.

**설계:**

```
Redis 키: "order-lock:{userId}"  SET NX EX 300
TTL 300초: 토큰 TTL과 동일 — 앱 크래시 시 안전망

주문 생성 흐름:
  1. GET entry-token → 토큰 일치 확인
  2. SET NX "order-lock:{userId}" EX 300 → 획득 실패 시 ORDER_ALREADY_IN_PROGRESS
  3. 주문 처리
  4a. 성공: DEL entry-token + DEL order-lock (best-effort)
  4b. 실패: DEL order-lock만 (토큰 보존 → 재시도 가능)
```

- `토큰 성격 (TTL 내 재시도 가능한 입장권)` 원칙 유지: 실패 시 토큰을 지우지 않아 재시도 가능.
- 성공 시 토큰 삭제 실패(Redis 일시 장애) → 락 유지 → entry-token 재사용을 TTL까지 차단 (의도적 보수 처리).
- 앱 크래시로 락 해제 못 할 경우 → TTL 300초 후 자동 만료 → 최대 5분 재시도 불가 (수용 가능한 trade-off).
